...
<head>
    ...
    function order() {
		...
		$(document).ready(function(){
			...
			if(localStorage.getItem('productAskInsertSW') == 'ON') {
				localStorage.removeItem('productAskInsertSW');
				localStorage.removeItem('productReturnOriginIdx');
			}
			// 문의 상세창에서 돌아왔다면, 해당 session을 없애준다.
			if(localStorage.getItem('productAskDetailSW') == 'ON') {
				localStorage.removeItem('productAskDetailSW');
				localStorage.removeItem('productReturnOriginIdx2');
			}
		});
		...
		function askInsert() {
			// 되돌아오기 세션값 주기
			localStorage.setItem('productAskInsertSW', 'ON');
			localStorage.setItem('productReturnOriginIdx', '${vo.idx}');
			
			location.href='${ctp}/about/askInsert?category=컬렉션상품&originIdx=${vo.idx}';
		}
