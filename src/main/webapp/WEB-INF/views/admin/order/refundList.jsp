...
<head>
	...
	<script>
    	...
    	location.href = "${ctp}/admin/order/refundList?pag=${pageVO.pag}&pageSize="+pageSize+"&sort=${sort}&search=${search}&searchString=${searchString}";
    }
    	...
    	location.href = "${ctp}/admin/order/refundList?search="+search+"&searchString="+searchString+"&sort="+sort;
		}
	...
	<div class="w3-main" style="margin-left:300px; margin-top:43px; padding-top:50px">
	 	...
	 	<div class="table-responsive" style="width:90%; margin:0px auto; padding:40px 50px 100px 50px" class="border">
	 		<div style="background-color:white; padding:20px; margin-bottom:30px">
	 			<div class="row">
	 				<div class="col text-left">
						<a class="btn btn-dark mb-4" href="${ctp}/admin/order/refundList" style="margin-left:20px;"><i class="fa-solid fa-arrows-rotate"></i></a>
	 				</div>
	 				<div class="col text-right">
	 				</div>
	 			</div>
				<div style="text-align:center"><span class="text-center" style="font-size:30px; text-align:center; font-weight:500">반품 관리</span></div>
			  <div style="text-align:center;">
				  <span class="text-center" style="font-size:15px; text-align:center; font-weight:300;">
					 	<br/><i class="fa-solid fa-check"></i>&nbsp;[배송완료 후, 7일 이내 반품 신청 가능] 반품신청 > 반품처리중(24시간 후) > 반품완료(120시간 후/5일 후) 로 자동 처리
					 	<br/><i class="fa-solid fa-check"></i>&nbsp;부분 반품의 경우에 나머지 상품은 자동으로 구매확정되며, 구매확정 포인트도 지급됩니다.
				  </span>
			  </div>
	 		</div>
			<div class="row" style="margin-top:50px">
		        ...
		        <option <c:if test="${sort == '반품진행중'}">selected</c:if> value="반품진행중">반품진행중</option>
		        <option <c:if test="${sort == '반품완료'}">selected</c:if> value="반품완료">반품완료</option>
		        <option <c:if test="${sort == '상품하자'}">selected</c:if> value="상품하자">(사유)상품하자</option>
		        <option <c:if test="${sort == '단순변심'}">selected</c:if> value="단순변심">(사유)단순변심</option>
		        <option <c:if test="${sort == '기타'}">selected</c:if> value="기타">(사유)기타</option>
		      </select>
				</div>
				<div class="col-5 text-right">
			    	...
			    	<div class="input-group">
				        ...
				        <select name="search" id="search" class="form-control">
				          <option <c:if test="${search == 'memNickname'}">selected</c:if> value="memNickname">반품 신청인(회원 별명)</option>
				          <option <c:if test="${search == 'refundCode'}">selected</c:if> value="refundCode">반품 코드</option>
				          <option <c:if test="${search == 'refundDetail'}">selected</c:if> value="refundDetail">반품 상세 사유</option>
				        </select>
				    	</div>
		  ...
		  <div class="table-responsive">
			    ...
			    <tbody>
			    	<c:if test="${empty vos}">
			    		<tr><td colspan="11" style="padding:30px"><b>반품 내역이 없습니다.</b></td></tr> 
			    	</c:if>
			    	<c:if test="${!empty vos}">
				    	<c:set var="curScrStartNo" value="${pageVO.curScrStartNo}" />
		 		    	<c:forEach var="vo" items="${vos}" varStatus="st">
					      <tr>
					        ...
					        <td>${vo.refundStatus}</td>
					        <td>${fn:substring(vo.refundDate,0,10)}</td>
					        <td>
					        	<button id="memInfo${vo.idx}" style="border:0px; background-color:transparent;" onclick="orderInfo('${vo.orderIdx}', '${vo.memNickname}')"><i class="fa-solid fa-circle-info" title="자세히" style="font-size:20px; color:#557A46"></i></button>
					        </td>
					      </tr>
