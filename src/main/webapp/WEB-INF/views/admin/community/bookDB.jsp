...
<head>
	...
	<script>
	    ...
	    if(evTarget.classList.contains("modal")) {
	    	$("#myModal").hide();
				location.href="${ctp}/admin/community/bookDB?pag=${pageVO.pag}&pageSize=${pageVO.pageSize}&search=${search2}&searchString=${searchString2}";
    ...
    function pageCheck() {
    	...
    	location.href = "${ctp}/admin/community/bookDB?pag=${pageVO.pag}&pageSize="+pageSize+"&search=${search2}&searchString=${searchString2}";
    }
...
/* 		function open() {
    	...
    	location.href = "${ctp}/admin/community/bookInsert?searchString="+searchString+"&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}&search2=${search2}&searchString2=${searchString2}&flag=bookDB";
		}
	 	...
	 	<div class="table-responsive" style="width:90%; margin:0px auto; padding:40px 50px 100px 50px" class="border">
		    ...
		    <tr>
		      ...
		      <td class="text-right">
		        ...
		        <c:if test="${pageVO.pag > 1}">
		          <a href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=1&search=${search2}&searchString=${searchString2}" title="첫페이지로">◁◁</a>
		          <a href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${pageVO.pag-1}&search=${search2}&searchString=${searchString2}" title="이전페이지로">◀</a>
		        </c:if>
		        ${pageVO.pag}/${pageVO.totPage}
		        <c:if test="${pageVO.pag < pageVO.totPage}">
		          <a href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${pageVO.pag+1}&search=${search2}&searchString=${searchString2}" title="다음페이지로">▶</a>
		          <a href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}&search=${search2}&searchString=${searchString2}" title="마지막페이지로">▷▷</a>
		        </c:if>
		      </td>
		    </tr>
		  </table>
		  ...
		  <div class="text-center">
			  <ul class="pagination justify-content-center">
			    <c:if test="${pageVO.pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=1&search=${search2}&searchString=${searchString2}"><i class="fa-solid fa-angles-left"></i></a></li></c:if>
			    <c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}&search=${search2}&searchString=${searchString2}"><i class="fa-solid fa-angle-left"></i></a></li></c:if>
			    <c:forEach var="i" begin="${pageVO.curBlock*pageVO.blockSize + 1}" end="${pageVO.curBlock*pageVO.blockSize + pageVO.blockSize}" varStatus="st">
			      <c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link text-white bg-secondary border-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${i}&search=${search2}&searchString=${searchString2}">${i}</a></li></c:if>
			      <c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${i}&search=${search2}&searchString=${searchString2}">${i}</a></li></c:if>
			    </c:forEach>
			    <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize + 1}&search=${search2}&searchString=${searchString2}"><i class="fa-solid fa-angle-right"></i></a></li></c:if>
			    <c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/admin/community/bookDB?pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}&search=${search2}&searchString=${searchString2}"><i class="fa-solid fa-angles-right"></i></a></li></c:if>
			  </ul>
		  </div>
